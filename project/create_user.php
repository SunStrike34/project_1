...
if (!empty($_POST)) {
    ...
    if (validateUserRequest($request) && checkEmailUserInBD($request['email'])) {
        addUserInBD($request);
    }else {
        $request = [];
        redirectАuthorized('create_user.php');
    }
    ...
    $request = [
        ...
        'job_title' => htmlspecialchars($_POST['job_title']) ?? null,
        ...
        'vk' => htmlspecialchars($_POST['vk']) ?? null,
        'telegram' => htmlspecialchars($_POST['telegram']) ?? null,
        'instagram' => htmlspecialchars($_POST['instagram']) ?? null
    ];

    editUser($request, $user['id']);

    if (isset($_FILES['image']) && checkError($_FILES['image']) && checkType($_FILES['image'])) { addImageInDirectory($_FILES['image'], $user['id']);}
